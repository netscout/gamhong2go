...
+------+
   ...
   v                 v                 v                 v                 v
+--------+      +--------+        +--------+        +--------+       +--------+
|  RAM   |      |  ROM   |        |  I/O   |        |Speaker |       |  LC +  |
| $0000- |      | $D000- |        | $C000- |        | $C030- |       |LC Sw.  |
| $BFFF  |      | $FFFF  |        | $C0FF  |        | $C03F  |       |$D000-$FFFF|
+--------+      +--------+        +--------+        +--------+       |$C080-$C08F|
                                                                     +--------+
```

Six devices are wired today (RAM, SoftSwitches, ROM, LC Card, LC Switches, Speaker); see `main.go:96-112` and `main.go:186`.

This walkthrough uses a three-device example (RAM/I/O/ROM) throughout §1-§8 because it fits in one glance; the real emulator wires six devices. See §8 footnote and `main.go:96-112` and `main.go:186` for the authoritative layout.
...
It is structurally identical to `cpu.Memory`. Because Go uses **structural typing** (duck typing by method set), the same concrete value -- `*memory.RAM`, `*memory.ROM`, `*io.SoftSwitches` -- automatically satisfies both interfaces without any explicit declaration. One type, two interfaces, zero boilerplate.
...
**Ordering matters.** The comment says it plainly: "Later mappings take priority over earlier ones." `Read` and `Write` both walk the slice backwards, so the last registered mapping that contains an address wins. This is called the **overlay pattern**, and it is the core idea that makes the entire design work. The simplified Apple II memory map below is constructed with a short sequence of `Map` calls, bottom layer first (the real emulator adds two more for the language card and one for the speaker; see §8):

To keep the first example small enough to fit in one glance, we show a simplified three-device wiring here. The actual machine wires six devices (`main.go:96-112`, `main.go:186`); §8 shows the full layout.

```text
bus.Map($0000, $BFFF, ram)    // #1  RAM, $0000-$BFFF (48 KB main memory)
bus.Map($C000, $C0FF, io)     // #2  Softswitch page, 256 bytes
bus.Map($D000, $FFFF, rom)    // #3  ROM, $D000-$FFFF (12 KB)
```
...
index  range              device
-----  -----------------  --------------------
  0    $0000..$BFFF       *memory.RAM
  1    $C000..$C0FF       *io.SoftSwitches
  2    $D000..$FFFF       *memory.ROM
```
...
- `$C000`-`$C0FF` (49,152-49,407): I/O wins (index 1).
- `$0000`-`$BFFF` (0-49,151): RAM wins (index 0, checked last).

`$C100`-`$CFFF` falls through to the open-bus fallback in the three-mapping example; in the real machine slot-card PROMs occupy that hole.
...
All three traces use the three-mapping setup from Section 4: RAM covering `$0000`-`$BFFF`, I/O at `$C000`-`$C0FF`, ROM at `$D000`-`$FFFF`.
...
mappings slice (len=3):
  [0] RAM  $0000..$BFFF  -> *memory.RAM
  [1] I/O  $C000..$C0FF  -> *io.SoftSwitches
...
loop i=1: m = mappings[1]  (I/O, $C000..$C0FF)
          $0042 >= $C000?  no
          skip
...
loop i=1: I/O $C000..$C0FF contains $0042? no
loop i=0: RAM $0000..$BFFF contains $0042? YES
          -> ram.Write($0042, $AB)
...
Sample output for the simplified three-device setup (using ASCII hyphens in this document):
...
Actual output on the running emulator (six devices; see `main.go:96-112` and `main.go:186`):

```text
  $0000-$BFFF  *memory.RAM
  $C000-$C0FF  *io.SoftSwitches
  $D000-$FFFF  *memory.ROM
  $D000-$FFFF  *languageCard.Card
  $C080-$C08F  *languageCard.Switches
  $C030-$C03F  *speaker.Device
```

The two `$D000-$FFFF` entries are not a bug -- LC is overlaid on top of ROM so reads are routed to LC, which delegates to ROM whenever `ramRead=false` (see `10.walkthrough_languagecard.md` §5; also the memory walkthrough §4 LC footnote in `03.walkthrough_memory.md`).

Reading this top-to-bottom (registration order), the last entry that covers any address wins. `$D042` is covered by both `$0000-$BFFF` (RAM, index 0) and `$D000-$FFFF` (ROM/LC), but the last matching entry wins, so LC wins in the real emulator. This output makes that clear at a glance.
...
As a first pass, the three-mapping example from §4 maps to three bus entries; the real emulator adds two more for the language card and one for the speaker (full list at `main.go:96-112`):

```text
Address range         Decimal           Wins via          Device type
--------------------  ----------------  ----------------  -----------------
$0000 - $BFFF         0 to 49151        index 0 (RAM)     *memory.RAM
$C000 - $C0FF         49152 to 49407    index 1 (I/O)     *io.SoftSwitches
$D000 - $FFFF         53248 to 65535    index 2 (ROM)     *memory.ROM
```

Here is the full routing picture with the mapping slice annotated (three-mapping example):
...
+-----Bus-----+
|  mappings:  |
|  [0] RAM    |  $0000-$BFFF (bottom layer)
|  [1] I/O    |  $C000-$C0FF (middle overlay)
...
**Four-read summary block** showing how the backward walk resolves each address (three-mapping example):
...
Read at $C080 (49,280):
  In the three-mapping example:
    i=2: ROM ($D000-$FFFF) contains $C080? NO
    i=1: I/O ($C000-$C0FF) contains $C080? YES -> read from I/O, done.
  In the real emulator, $C080-$C08F is overlaid by the language-card
  softswitch device (see `10.walkthrough_languagecard.md` §4 and §6; main.go:112), which wins via the same last-mapping-wins
  rule -- LC Switches (index 4) is registered after sw, so it wins.
...
Read at $FFFF (65,535): same range as $D042 -- ROM wins (LC wins in real emulator).
```

**Real emulator wiring (`main.go:96-112` and `main.go:186`):**
...
-----  -----------------  --------------------------
  ...
  1    $C000..$C0FF       *io.SoftSwitches
  2    $D000..$FFFF       *memory.ROM
  3    $D000..$FFFF       *languageCard.Card
  4    $C080..$C08F       *languageCard.Switches
  5    $C030..$C03F       *speaker.Device
```
...
Simplicity and generality. A slice works for any range granularity, including sub-page ranges like `$C000`-`$C0FF` (49,152-49,407) for I/O softswitches. A 256-entry page table -- one entry per 256-byte page -- would make lookup O(1) at the cost of more complex setup code and a table that occupies at least one separate cache line. For a machine with fewer than ten mappings, the current design is cache-friendly, simpler to read, and measurably fast enough. The right time to add a page-table optimization is when a profiler says so -- not now.
...
The next walkthroughs cover four devices that have already shipped: `05.walkthrough_io.md` (the Apple II's I/O region at `$C000-$C0FF`, whose reads and writes have *side effects* beyond storing bytes -- e.g. reading `$C030` produces a speaker click), `10.walkthrough_languagecard.md` (a 16 KB RAM bank that overlays `$D000-$FFFF`, controlled by softswitches at `$C080-$C08F`), `11.walkthrough_speaker.md` (the speaker click device at `$C030-$C03F`), and the four-part disk series `12.walkthrough_disk_image.md` / `13.walkthrough_disk_gcr.md` / `14.walkthrough_disk_drive.md` / `15.walkthrough_disk_controller.md` (the Disk II card's softswitches and slot PROM). The wiring in `main.go` that stacks these onto the bus is at `main.go:96-112` and `main.go:186`.

The remaining device walkthroughs cover video (`video/`) -- each following the same pattern: one new type, one or a few `Map` calls, zero changes to `bus.go`.

Everything in this section's "What Comes Next" used to be future work; as of 2026-04-20 they are all implemented, with six devices (seven when a disk card is installed via the lazy-install closure documented in `15.walkthrough_disk_controller.md` §7) sharing the bus via the same `Map`-then-backward-walk protocol you just learned.
